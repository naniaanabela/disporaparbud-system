...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 21, 2021 at 11:23 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_pariwisata`  AS SELECT `p`.`id_pariwisata` AS `id_pariwisata`, `p`.`tahun` AS `tahun`, `p`.`objek_wisata` AS `objek_wisata`, `p`.`wisnus` AS `wisnus`, `p`.`wisman` AS `wisman`, `p`.`sarana_prasarana` AS `sarana_prasarana`, `p`.`daya_tarik` AS `daya_tarik`, (select ifnull(`tb_administrator`.`id_administrator`,NULL) from `tb_administrator` where `tb_administrator`.`id_administrator` = `p`.`ditambahkan_oleh`) AS `id_administrator`, (select ifnull(`tb_administrator`.`nama_lengkap`,NULL) from `tb_administrator` where `tb_administrator`.`id_administrator` = `p`.`ditambahkan_oleh`) AS `nama_lengkap`, (select ifnull(`tb_administrator`.`username`,NULL) from `tb_administrator` where `tb_administrator`.`id_administrator` = `p`.`ditambahkan_oleh`) AS `username`, (select ifnull(`tb_administrator`.`kata_sandi`,NULL) from `tb_administrator` where `tb_administrator`.`id_administrator` = `p`.`ditambahkan_oleh`) AS `kata_sandi` FROM `tb_pariwisata` AS `p` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_perhitungan`  AS SELECT `p`.`id_perhitungan` AS `id_perhitungan`, `p`.`tanggal_perhitungan` AS `tanggal_perhitungan`, `p`.`data_olah` AS `data_olah`, `p`.`normalisasi` AS `normalisasi`, `p`.`tahun` AS `tahun`, `p`.`jumlah_cluster` AS `jumlah_cluster`, `p`.`learning_rate` AS `learning_rate`, `p`.`jumlah_iterasi` AS `jumlah_iterasi`, (select ifnull(`tb_administrator`.`id_administrator`,NULL) from `tb_administrator` where `tb_administrator`.`id_administrator` = `p`.`ditambahkan_oleh`) AS `id_administrator`, (select ifnull(`tb_administrator`.`nama_lengkap`,NULL) from `tb_administrator` where `tb_administrator`.`id_administrator` = `p`.`ditambahkan_oleh`) AS `nama_lengkap`, (select ifnull(`tb_administrator`.`username`,NULL) from `tb_administrator` where `tb_administrator`.`id_administrator` = `p`.`ditambahkan_oleh`) AS `username`, (select ifnull(`tb_administrator`.`kata_sandi`,NULL) from `tb_administrator` where `tb_administrator`.`id_administrator` = `p`.`ditambahkan_oleh`) AS `kata_sandi` FROM `tb_perhitungan` AS `p` ;
...
--
ALTER TABLE `tb_pariwisata`
  MODIFY `id_pariwisata` bigint(20) NOT NULL AUTO_INCREMENT;
